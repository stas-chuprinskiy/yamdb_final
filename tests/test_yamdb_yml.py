...
class TestWorkflow:
    ...
    def test_workflow(self):
        ...
        if not is_yaml and not is_yml:
            assert False, (
                f'В каталоге {workflow_dir} не найден файл с описанием workflow '
                f'{yaml} или {yml}.\n'
        ...
        if is_yaml and is_yml:
            assert False, (
                f'В каталоге {workflow_dir} не должно быть двух файлов {yamdb_workflow_basename} '
        ...
        try:
            ...
            assert False, f'Проверьте, что добавили файл {filename} в каталог {workflow_dir} для проверки'
